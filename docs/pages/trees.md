...
* * *
#### Disclaimer: you can copy the examples below; they'll work fine if you have loaded the necessary packages in the preamble. The working examples are available [here](https://github.com/DOzernyi/LSA_templates/blob/master/docs/pages/examples/examples.pdf).
* * *
...
\begin{multicols}{2}
% trivially, this is multicols; {2} at the end indicates the number of columns you need --
% {2} is the default setting if you forget to specify;
% but LaTeX will likely be grumpy and throw something like "Missing number, treated as zero" at you
% \scriptsize
...
% but since this was pretty much abandoned for the reasons Carnie explains in "Constituent Structure", "\\" just creates a line break
\end{forest}
\columnbreak
%\scriptsize
\a Tree 2
...
[V'
% note the use of \varnothing, which is mathematical, and so is enclosed in $...$;
% do not use \{\} or (particularly) \[\] for in-line math
...
% looseness is useful when an arrow from a lower node to an upper node crosses the tree --
% loosing it leads it out of the tree; there are other more elegant options not to be explored here
...
```tex
\dots
% You can use this to get "..." if you want to skip parts of a larger structure.   

% Arrows can have text on them; here' an example from the package documentation
{\draw[<->] () .. controls +(left:1cm) and +(south west:0.4cm) ..
  node[very near start,below,sloped]{\tiny agree} (!us);}
```

Do consult the documentation to customize the use!
